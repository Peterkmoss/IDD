...
select product, count, rank
from (
	select product_id as product, count_product_orders as count, rank() over (order by -count_product_orders) as rank
	from product_order_count
	order by -count_product_orders) tmp
where rank <= 10;